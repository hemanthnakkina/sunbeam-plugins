...
class EnableHeatApplicationStep(BaseStep, JujuStepHelper):
    ...
    def run(self, status: Optional[Status] = None) -> Result:
        ...
        try:
            ...
            tfvars = {}
        tfvars.update({"enable-heat": True})
...
class DisableHeatApplicationStep(BaseStep, JujuStepHelper):
    ...
    def run(self, status: Optional[Status] = None) -> Result:
        ...
        tfvars = read_config(self.client, self._CONFIG)
        tfvars.update({"enable-heat": False})
        update_config(self.client, self._CONFIG, tfvars)
        self.tfhelper.write_tfvars(tfvars)
...
class HeatPlugin(EnableDisablePlugin):
    version = Version("0.0.1")

    def __init__(self) -> None:
        super().__init__(name="heat")
        self.snap = Snap()
        self.tfplan = "deploy-openstack"

    def pre_enable(self):
        src = self.snap.paths.snap / "etc" / self.tfplan
    ...
    def run_enable_plans(self):
        ...
        plan = [
            ...
            EnableHeatApplicationStep(tfhelper, jhelper),
        ]

        run_plan(plan, console)
        click.echo("OpenStack Heat application enabled.")
    ...
    def run_disable_plans(self):
        ...
        plan = [
            TerraformInitStep(tfhelper),
            DisableHeatApplicationStep(tfhelper, jhelper),
        ]

        run_plan(plan, console)
        click.echo("OpenStack Heat application disabled.")
